...
from 服务.agent import 模型网关
from 服务.agent.状态 import AgentState
from 服务.agent.工具 import (
    KNOWLEDGE_POINTS,
    QUESTION_BANK,
    get_scaffold_hint,
    grade_answer,
...
def load_profile(state: AgentState) -> AgentState:
    data = 数据库.load_student(state["student_id"]) or {
...
def tutor_with_scaffolding(state: AgentState) -> AgentState:
    ...
    reply, provider = 模型网关.complete(system, state.get("message", ""))
    state["reply"] = reply
