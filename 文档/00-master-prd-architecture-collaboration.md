...
适用场景：2026-08-18 晚上课堂讨论、学生创意方案收集、五人分工开发启动、9 月 9 日初赛提交前集成验收。
...
这份文档现在是 `v0.3 课前讨论版`，不是最终锁死版。它先解决三个问题：

1. 让老师今晚能投屏讲清楚项目方向。
2. 让五位同学知道 A-E 各自大概负责什么。
3. 让学生的创意方案能按统一格式收集，方便课后整合。

今晚看完学生方案后，老师会决定哪些创意采纳、延期或暂不做。确认后再升级到 `v0.4 方案整合版` 或 `v1.0 开发基线版`。一旦进入 `v1.0`，学生正式开发就以新版文档为准。

## 2.1 课堂讲解与学习材料

- 老师课堂投屏优先使用：[课堂展示讲义](01-classroom-student-handbook.md) 和 `文档/presentation/ai-study-课堂展示讲义-v0.3.docx`。
...
- 每位同学领取任务前必须阅读：[文档领取、版本升级与课后确认流程](03-document-reading-and-versioning-guide.md)。
- 今晚提交创意方案使用：[学生创意方案模板](proposals/student-proposal-template.md)。老师课后整合使用：[课后最终方案确认模板](proposals/final-decision-template.md)。
...
```text
应用/web/                         # 角色 E：前端、演示页
服务/api/                     # 角色 A：FastAPI 网关和接口
服务/agent/                   # 角色 A：LangGraph 工作流
包/contracts/               # 角色 A/E：共享类型、OpenAPI 生成物
数据/knowledge_graph/math/        # 角色 B：数学知识图谱
数据/knowledge_graph/chinese/     # 角色 C：语文知识图谱
数据/knowledge_graph/english/     # 角色 D：英语知识图谱
数据/question_bank/math/          # 角色 B：数学题库
数据/question_bank/chinese/       # 角色 C：语文题库
数据/question_bank/english/       # 角色 D：英语题库
提示词库/math/                     # 角色 B：数学 Prompt
提示词库/chinese/                  # 角色 C：语文 Prompt
提示词库/english/                  # 角色 D：英语 Prompt
文档/api/                         # 接口契约
文档/roles/                       # 五人执行文档
文档/proposals/                   # 学生创意方案和老师最终决策
文档/03-document-reading-and-versioning-guide.md
文档/CHANGELOG.md                 # 文档版本记录
测试/                            # 单元测试、接口测试、端到端测试
```
...
- [文档领取、版本升级与课后确认流程](03-document-reading-and-versioning-guide.md)
- [A-E 角色分工确认看板](roles/00-role-assignment-board.md)
...
| 日期 | 目标 | 验收 |
| --- | --- | --- |
| 2026-08-18 晚 | 课堂确认角色、MVP 范围、AI 精灵方向、学生创意方案 | 文档从 v0.3 升级为 v0.4 或 v1.0 |
...
每个创意方案必须按 `文档/proposals/student-proposal-template.md` 写成 1 页以内，建议保存到：

```text
文档/proposals/student-proposals/role-X-姓名-创意名称.md
```

还没确认姓名时，可以先用：

```text
文档/proposals/student-proposals/role-X-pending-idea.md
```
...
老师课后整合时，把每个创意标记为 `采纳`、`延期`、`暂不做`。被采纳的创意必须写清楚它归哪个角色负责、改哪个接口或页面、怎么验收。最终决策写入 `文档/proposals/final-decision.md`，再同步更新本文档和 A-E 角色文档。
...
- 学生创意方案哪些采纳、延期、暂不做。
- MVP 年级范围和第一周最小提交。
- 是否需要把新版课堂讲义同步生成 Word/PDF。

## 16. 课后升级文档的固定顺序

今晚确认完以后，按这个顺序更新，避免学生看不同版本：

1. 新建或更新 `文档/proposals/final-decision.md`，记录老师最终选择。
2. 更新本文档，把采纳的创意写进 PRD、架构和验收标准。
3. 更新 `文档/roles/00-role-assignment-board.md` 和 A-E 角色文档。
4. 如果接口字段变了，先改 `文档/api/openapi-contract-v0.yaml`。
5. 更新 `文档/01-classroom-student-handbook.md`，保持课堂讲解口径一致。
6. 重新生成 Word 投屏讲义。
7. 更新 `文档/CHANGELOG.md`，写清楚版本变化。

这一步先预留口子。等老师看完学生方案后，只需要按这个流程补最终信息，不需要推翻现在的工程结构。
