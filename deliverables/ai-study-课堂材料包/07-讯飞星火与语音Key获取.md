...
本项目不把讯飞星火或讯飞语音作为硬性依赖。赛题公开文本强调“利用大模型与 Agent 技术”，没有写明只能使用星火。考虑到主办/举办方包含科大讯飞，建议至少保留一个星火或讯飞语音展示点，作为赛事契合亮点。
...
- 讯飞星火：赛事契合、中文教育场景友好。
- DeepSeek：推理和讲解生成可作为主力或备用。
- Qwen：中文与多模态生态可作为备用。
- MiniMax：长文本、语音或多模态能力可作为扩展。
...
代码中不能把某一家模型写死。提交材料中要说明每个模型分别承担什么任务，例如：
...
1. 文本大模型能力优先接通，具体供应商可配置。
2. 星火和讯飞语音作为加分项，能接就展示，不能接就保留 mock。
