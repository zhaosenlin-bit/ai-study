...
4. 是否必须接入讯飞星火、讯飞 ASR/TTS；如果账号暂时不稳定，是否允许用兼容模型做备用。
...
| 层级 | MVP 选择 | 原因 |
| --- | --- | --- |
| 前端 | React + Vite + TypeScript 或 Vue3 + Vite | 两者都可，角色 E 按团队熟悉度选；必须组件化 |
| 后端 | Python FastAPI | OpenAPI 自动生成，适合 Agent/RAG 接入 |
| Agent | LangGraph 优先，Dify 作为可视化备用 | 更容易体现规划、记忆、工具调用 |
| 模型 | 讯飞星火主用，DeepSeek/Qwen 备用 | 赛事契合 + 稳定兜底 |
